...
def draw_cmc_plot(cmc_df, cmc_df_1, output_folder="cmc", FOLDS = 10):
    """
    ...
    models = ['TattTRN(EfficientNetv2)', 'EfficientNetv2']

    display = {
        "TattTRN(EfficientNetv2)": {"name": "TattTRN(EfficientNetv2)", "color": "red", "marker": 'x'},
        "EfficientNetv2": {"name": "EfficientNetv2", "color": "blue", "marker": 'x'},
    }
    ...
    ax1.set_title('WebTattoo', fontsize=16)
    ax2.set_title('BIVTatt', fontsize=16)
    ...
    for model in models:
        ...
        sns.lineplot(data=cmc_model, x="rank", y="accuracy",
                        ...
                        color=display[model_name]["color"], marker=display[model_name]["marker"], ax=ax1)
    ...
    for model in models:
        ...
        # For each model+experiment, we select the cmc values assosiated with it
        cmc_model = cmc_df_1[cmc_df_1["model"] == model_name]
        for c in cmc_model['accuracy']:
            print(c/100)
        # Then we plot the cmc values using errorbars. Usually we have multiple folds in cmc_model, i.e. we have multiple values for each rank.
        # Using sns.lineplot with errorbars automaticly computes the mean and std of each rank accuracy and plots it.
        sns.lineplot(data=cmc_model, x="rank", y="accuracy",
                        errorbar='sd', markersize=3, label=display[model_name]["name"],
                        color=display[model_name]["color"], marker=display[model_name]["marker"], ax=ax2)
    ...
    plt.ylabel("Identification Rate (%)", fontsize=13)
    plt.xlabel("", fontsize=15)
    plt.yticks([80, 85, 90, 95], fontsize=12)
    ...
    plt.ylabel("Identification Rate (%)", fontsize=13)
    plt.yticks([95, 96, 97, 98, 99, 100], fontsize=12)
    plt.grid(linestyle='--', linewidth=0.3)
    plt.tight_layout()
...
def main(args):
    ...
    cmc_df_1 = pd.DataFrame(columns=["rank", "accuracy", "model"])

    for f in range(args.fold):
        # combined
        scores_path_comb = os.path.join(args.combined_scores_dir, 'close_set_scores_{}.txt'.format(f))
        tp_path_comb = os.path.join(args.combined_scores_dir, 'close_set_scores_tp_{}.txt'.format(f))
        ...
        cmc_df = cmc_df.append(pd.DataFrame(
            {"rank": range(1, args.r+1), "accuracy": np.array(cmc) * 100, "model": "TattTRN(EfficientNetv2)"}), ignore_index=True)

        # top
        scores_path_top = os.path.join(args.top_scores_dir, 'close_set_scores_{}.txt'.format(f))
        tp_path_top = os.path.join(args.top_scores_dir, 'close_set_scores_tp_{}.txt'.format(f))
        ...
        # combined
        scores_path_comb = os.path.join(args.combined_scores_dir_1, 'close_set_scores_{}.txt'.format(f))
        tp_path_comb = os.path.join(args.combined_scores_dir_1, 'close_set_scores_tp_{}.txt'.format(f))
        scores_comb = load_scores_from_file(scores_path_comb, tp_path_comb)
        ranks_comb = get_cmc_curve(scores_comb, args.r)
        cmc = ranks_comb[:args.r]
        # Append cmc results to dataframe, stating the ranks, accuracy and model+experiment name
        cmc_df_1 = cmc_df_1.append(pd.DataFrame(
            {"rank": range(1, args.r+1), "accuracy": np.array(cmc) * 100, "model": "TattTRN(EfficientNetv2)"}), ignore_index=True)

        # top
        scores_path_top = os.path.join(args.top_scores_dir_1, 'close_set_scores_{}.txt'.format(f))
        tp_path_top = os.path.join(args.top_scores_dir_1, 'close_set_scores_tp_{}.txt'.format(f))
        scores_top = load_scores_from_file(scores_path_top, tp_path_top)
        ranks_top = get_cmc_curve(scores_top, args.r)
        cmc = ranks_top[:args.r]
        # Append cmc results to dataframe, stating the ranks, accuracy and model+experiment name
    ...
    draw_cmc_plot(cmc_df, cmc_df_1, plots_dir, args.r)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--combined_scores_dir', type=str, 
    ...
    parser.add_argument('--top_scores_dir', type=str, 
                        default='/Users/soler/Research_Projects/Others-Projects/Github-projects/tattoo-retrieval/models/baselines/efficientnet_v2_s_0.5_0.1_512_top/02261304/scores',
                        help="scores directory for top pipeline")
    parser.add_argument('--combined_scores_dir_1', type=str, 
                        default='/Users/soler/Research_Projects/Others-Projects/Github-projects/tattoo-retrieval/models/efficientnet_v2_s_4.0_0.5_256/03041525/scores',
                        help="scores directory for combined pipeline")
    parser.add_argument('--top_scores_dir_1', type=str, 
                        default='/Users/soler/Research_Projects/Others-Projects/Github-projects/tattoo-retrieval/models/baselines/efficientnet_v2_s_0.5_0.5_256_top/03041817/scores',
                        help="scores directory for top pipeline")

    parser.add_argument('--root_dir', type=str, 
                        default='/Users/soler/Research_Projects/Others-Projects/Github-projects/tattoo-retrieval/results/comparison',
